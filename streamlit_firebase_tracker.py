...
if st.session_state.needs_rerun:
    ...
    st.rerun().date.unique()
                selected_date = st.sidebar.selectbox("Select date to edit/delete:", dates)
                
                # Get the row index for the selected date
                date_idx = st.session_state.current_user_data[st.session_state.current_user_data['Date'].dt# streamlit_firebase_tracker.py
    ...
    with tab2:
        ...
        if st.session_state.current_user_data.empty:
            ...
            if len(date_range) == 2:
                ...
                st.info("Please select both start and end dates.")# streamlit_firebase_tracker.py
import streamlit as st
import pandas as pd
import numpy as np
from datetime import datetime, timedelta
