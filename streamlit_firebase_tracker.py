st.info("💡 **Tip:** For large batches, consider harvesting 15-25 bakul per day for optimal quality.")
    ...
    with history_tab:
        ...
        # Harvest Summary by Flower Date (existing)
        st.subheader("🌸 Harvest Summary by Flower Date")
        
        harvest_summary = {}
        for harvest in user_harvests:
            flower_date = harvest['flower_date']
            if flower_date not in harvest_summary:
                harvest_summary[flower_date] = {
                    'expected_bakul': harvest['flower_total_bakul'],
                    'harvested_bakul': 0,
                    'harvest_count': 0,
                    'harvest_period': None,
                    'first_harvest_days': None,
                    'completed': False,
                    'completed_marked': False
                }
            
            summary = harvest_summary[flower_date]
            summary['harvested_bakul'] += harvest.get('equivalent_bakul', harvest.get('total_harvest_bakul', 0))
            summary['harvest_count'] += 1
            
            if summary['harvest_period'] is None:
                summary['harvest_period'] = harvest['harvest_date']
            else:
                start_date = min(summary['harvest_period'], harvest['harvest_date'])
                end_date = max(summary['harvest_period'], harvest['harvest_date'])
                if start_date == end_date:
                    summary['harvest_period'] = start_date
                else:
                    summary['harvest_period'] = start_date + " to " + end_date
            
            if summary['first_harvest_days'] is None:
                flower_date_obj = datetime.strptime(flower_date, '%Y-%m-%d')
                harvest_date_obj = datetime.strptime(harvest['harvest_date'], '%Y-%m-%d')
                summary['first_harvest_days'] = (harvest_date_obj - flower_date_obj).days
            
            if harvest.get('completed', False):
                summary['completed'] = True
            if harvest.get('marked_completed', False):
                summary['completed_marked'] = True
        
        if harvest_summary:
            summary_df_data = []
            for flower_date, summary in harvest_summary.items():
                efficiency = (summary['harvested_bakul'] / summary['expected_bakul']) * 100 if summary['expected_bakul'] > 0 else 0
                
                status_parts = []
                if summary['completed']:
                    status_parts.append("Completed")
                if summary['completed_marked']:
                    status_parts.append("(Marked)")
                status = " ".join(status_parts) if status_parts else ""
                
                summary_df_data.append({
                    'Flower Date': flower_date,
                    'Expected Bakul': summary['expected_bakul'],
                    'Harvested Bakul': summary['harvested_bakul'],
                    'Efficiency (%)': f"{efficiency:.1f}%",
                    'Status': status,
                    'Harvest Count': summary['harvest_count'],
                    'Harvest Period': summary['harvest_period'],
                    'Days to First Harvest': summary['first_harvest_days']
                })
            
            summary_df = pd.DataFrame(summary_df_data)
            st.dataframe(summary_df, use_container_width=True, hide_index=True)
            
            total_expected = sum(s['expected_bakul'] for s in harvest_summary.values())
            total_harvested = sum(s['harvested_bakul'] for s in harvest_summary.values())
            completed_batches = sum(1 for s in harvest_summary.values() if s['completed'])
            total_batches = len(harvest_summary)
            avg_efficiency = (total_harvested / total_expected) * 100 if total_expected > 0 else 0
            
            col1, col2, col3, col4, col5 = st.columns(5)
            with col1:
                st.metric("Flower Batches", f"{completed_batches}/{total_batches}")
            with col2:
                st.metric("Completed", f"{completed_batches}/{total_batches}")
            with col3:
        ...
        # NEW: Daily Harvest Summary by Harvest Date
        st.subheader("📅 Daily Harvest Summary by Harvest Date")
        ...
        for harvest in user_harvests:
            ...
            daily_summary = daily_harvest_summary[harvest_date]
            harvest_distribution = harvest.get('harvest_bakul_distribution', {})
            for size in HARVEST_FRUIT_SIZES:
                daily_summary[size] += harvest_distribution.get(size, 0)
            daily_summary['total_bakul'] += harvest.get('total_harvest_bakul', 0)
        
        if daily_harvest_summary:
            daily_df_data = []
            for harvest_date in sorted(daily_harvest_summary.keys(), reverse=True):
                daily = daily_harvest_summary[harvest_date]
                daily_df_data.append({
                    'Harvest Date': harvest_date,
                    'Total Bakul': daily['total_bakul'],
                    '>600g': daily['>600g'],
                    '>500g': daily['>500g'],
                    '>400g': daily['>400g'],
                    '>300g': daily['>300g'],
            ...
            total_daily_bakul = sum(d['total_bakul'] for d in daily_harvest_summary.values())
            total_600g = sum(d['>600g'] for d in daily_harvest_summary.values())
            total_500g = sum(d['>500g'] for d in daily_harvest_summary.values())
            total_400g = sum(d['>400g'] for d in daily_harvest_summary.values())
            total_300g = sum(d['>300g'] for d in daily_harvest_summary.values())
            total_reject = sum(d['Reject'] for d in daily_harvest_summary.values())
            harvest_days = len(daily_harvest_summary)
            
            st.write("**Overall Daily Harvest Summary:**")
            col1, col2, col3, col4, col5, col6, col7 = st.columns(7)
            with col1:
                st.metric("Harvest Days", harvest_days)
            with col2:
                st.metric("Total Bakul", f"{total_daily_bakul}")
            with col3:
                st.metric(">600g", f"{total_600g}")
            with col4:
                st.metric(">500g", f"{total_500g}")
            with col5:
                st.metric(">400g", f"{total_400g}")
            with col6:
                st.metric(">300g", f"{total_300g}")
            with col7:
                st.metric("Reject", f"{total_reject}")
        else:
            st.info("No daily harvest data available")
        
        st.markdown("---")
        
        # All Harvest Records (Detailed)
